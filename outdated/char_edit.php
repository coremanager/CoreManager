...
//########################################################################################################################
function edit_char() {
 global $lang_global, $lang_char, $lang_item, $output, $logon_db, $characters_db, $realm_id, $arcm_db, 
...
$result = $sql->query("SELECT acct FROM `characters` WHERE guid = '$id'");

if ($sql->num_rows($result)){
  //resrict by owner's gmlvl
  $owner_acc_id = $sql->result($result, 0, 'acct');
  //this_is_junk: oh hello, non-standard naming. <_<
  $sql->connect($logon_db['addr'], $logon_db['user'], $logon_db['pass'], $logon_db['name']);
  $query = $sql->query("SELECT gm,login FROM accounts WHERE acct ='$owner_acc_id'");
  $owner_gmlvl = $sql->result($query, 0, 'gm');
  $owner_name = $sql->result($query, 0, 'login');
  $owner_check = $sql->result($query, 0, 'login');

 if ($user_lvl >= $owner_gmlvl){
  $sql->connect($characters_db[$realm_id]['addr'], $characters_db[$realm_id]['user'], $characters_db[$realm_id]['pass'], $characters_db[$realm_id]['name']);

  $result = $sql->query("SELECT guid,acct,data,name,race,class,positionx,positiony,mapid,online,SUBSTRING_INDEX(SUBSTRING_INDEX(playedtime,' ',2),' ',-1) AS totaltime,positionz,zoneid, level, gender  FROM `characters` WHERE guid = '$id'");
  ...
  {
    $query = $sql->query("SELECT guildname FROM guilds WHERE guildid ='{$char_data[PLAYER_GUILDID]}'");
    $guild_name = $sql->result($query, 0, 'guildname');
    if ($user_lvl > 0 )
    ...
    {
      $guild_rank_query = $sql->query("SELECT rankname FROM guild_ranks WHERE guildid ='{$char_data[PLAYER_GUILDID]}' AND rid='{$char_data[PLAYER_GUILDRANK]}'");
  ...
  $crit = round($crit[1],4);
  $range_crit = unpack("f", pack("L", $char_data[PLAYER_RANGED_CRIT_PERCENTAGE]));
...
<tr>
 <td colspan=\"8\">{$lang_char['honor_points']}: <input type=\"text\" name=\"honor_points\" size=\"8\" maxlength=\"6\" value=\"{$char_data[PLAYER_FIELD_HONOR_CURRENCY]}\" />/
 <input type=\"text\" name=\"arena_points\" size=\"8\" maxlength=\"6\" value=\"{$char_data[PLAYER_FIELD_ARENA_CURRENCY]}\" /> - {$lang_char['honor_kills']}: <input type=\"text\" name=\"total_kills\" size=\"8\" maxlength=\"6\" value=\"{$char_data[PLAYER_FIELD_LIFETIME_HONORBALE_KILLS]}\" /></td>
</tr>
  <tr>
    <td width=\"2%\"><input type=\"checkbox\" name=\"check[]\" value=\"a0\" /></td><td width=\"18%\">{$lang_item['head']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD])."</a></td>
    <td width=\"15%\">{$lang_item['health']}:</td><td width=\"15%\"><input type=\"text\" name=\"health\" size=\"10\" maxlength=\"6\" value=\"{$char_data[UNIT_FIELD_HEALTH]}\" /></td>
    <td width=\"15%\">{$lang_item['res_holy']}:</td><td width=\"15%\"><input type=\"text\" name=\"res_holy\" size=\"10\" maxlength=\"4\" value=\"{$char_data[UNIT_FIELD_RESISTANCES + 1]}\" /></td>
    <td width=\"18%\">{$lang_item['gloves']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+18]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+18])."</a></td><td width=\"2%\"><input type=\"checkbox\" name=\"check[]\" value=\"a9\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a1\" /></td><td>{$lang_item['neck']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+2]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+2])."</a></td>
    ...
    <td>{$lang_item['belt']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+10]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+10])."</a></td> <td><input type=\"checkbox\" name=\"check[]\" value=\"a5\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a2\" /></td><td>{$lang_item['shoulder']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+4]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+4])."</a></td>
    ...
    <td>{$lang_item['legs']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+12]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+12])."</a></td><td><input type=\"checkbox\" name=\"check[]\" value=\"a6\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a14\" /></td><td>{$lang_item['back']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+28]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+28])."</a></td>
    ...
    <td>{$lang_item['feet']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+14]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+14])."</a></td><td><input type=\"checkbox\" name=\"check[]\" value=\"a7\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a4\" /></td><td>{$lang_item['chest']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+8]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+8])."</a></td>
    ...
    <td>{$lang_item['finger']} 1<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+20]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+20])."</a></td><td><input type=\"checkbox\" name=\"check[]\" value=\"a10\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a3\" /></td><td>{$lang_item['shirt']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+6]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+6])."</a></td>
    ...
    <td>{$lang_item['finger']} 2<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+22]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+22])."</a></td><td><input type=\"checkbox\" name=\"check[]\" value=\"a11\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a18\" /></td><td>{$lang_item['tabard']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+36]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+36])."</a></td>
    ...
    <td>{$lang_item['trinket']} 1<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+24]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+24])."</a></td><td><input type=\"checkbox\" name=\"check[]\" value=\"a12\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a8\" /></td><td>{$lang_item['wrist']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+16]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+16])."</a></td>
    ...
    <td>{$lang_item['trinket']} 2<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+26]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+26])."</a></td><td><input type=\"checkbox\" name=\"check[]\" value=\"a13\" /></td>
  </tr>
  <tr>
    <td><input type=\"checkbox\" name=\"check[]\" value=\"a15\" /></td>
    <td colspan=\"2\">{$lang_item['main_hand']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+30]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+30])."</a></td>
    <td colspan=\"2\"><input type=\"checkbox\" name=\"check[]\" value=\"a16\" />&nbsp;{$lang_item['off_hand']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+32]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+32])."</a></td>
    <td colspan=\"2\">{$lang_item['ranged']}<br /><a href=\"$item_datasite{$char_data[PLAYER_FIELD_INV_SLOT_HEAD+34]}\" target=\"_blank\">".get_item_name($char_data[PLAYER_FIELD_INV_SLOT_HEAD+34])."</a></td>
...
| {$lang_char['ranged_crit']}: <input type=\"text\" name=\"range_crit\" size=\"3\" maxlength=\"14\" value=\"$range_crit\" />%</td>
 ...
 <tr>
<td colspan=\"4\">{$lang_char['gold']}: <input type=\"text\" name=\"money\" size=\"10\" maxlength=\"8\" value=\"{$char_data[PLAYER_FIELD_COINAGE]}\" /></td>
  <td colspan=\"4\">{$lang_char['tot_play_time']}: <input type=\"text\" name=\"tot_time\" size=\"8\" maxlength=\"14\" value=\"{$char[10]}\" /></td>
</tr>
<tr>
...
//inventory+bank items
  $sql->connect($characters_db[$realm_id]['addr'], $characters_db[$realm_id]['user'], $characters_db[$realm_id]['pass'], $characters_db[$realm_id]['name']);
  $query2 = $sql->query("SELECT containerslot,slot,entry FROM playeritems WHERE ownerguid = '$id' ORDER BY containerslot,slot");
...
function do_edit_char() {
 global $lang_global, $lang_char, $output, $logon_db,
...
//we cannot edit online chars
 if(!$sql->result($result, 0, 'online')){
  //resrict by owner's gmlvl
  $owner_acc_id = $sql->result($result, 0, 'accounts');
  //more non-standard variable naming. yay
  $sql->connect($logon_db['addr'], $logon_db['user'], $logon_db['pass'], $logon_db['name']);
  $query = $sql->query("SELECT gm FROM accounts WHERE acct ='$owner_acc_id'");
  $owner_gmlvl = $sql->result($query, 0, 'gm');
  $new_owner_name = $_GET['owner_name'];
  $query = $sql->query("SELECT acct FROM accounts WHERE login ='$new_owner_name'");
