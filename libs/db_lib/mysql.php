...
class SQL //MySQL
{
    ...
    if ($this->link_id){
      if($db_name){
        if (@mysql_select_db($db_name, $this->link_id)) return $this->link_id;
          else die(error($db_name."<br />".mysql_error()."<br />".$lang_global['err_sql_open_db']." ('$db_name')"));
        if (!empty($use_names)) $this->query("SET NAMES '$use_names'");
      }
    } else die(error($db_name."<br />".mysql_error()."<br />".$lang_global['err_sql_conn_db']));
  }
  ...
  function query($sql){
    ...
    if ($this->query_result){
      ...
      die(error($sql."<br />".mysql_error($this->link_id)));
      return false;
    }
  }
